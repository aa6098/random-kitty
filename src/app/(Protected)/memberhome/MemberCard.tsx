...
import { MapPinIcon, HeartIcon, EnvelopeSimpleIcon } from "@phosphor-icons/react"
      ...
      <Link href={`/members/${id}`} className="group">
        <Card className="overflow-hidden transition-shadow group-hover:shadow-md">
          <div className="relative aspect-square w-full overflow-hidden bg-muted">
            ...
            {/* Email button — bottom-left, sibling of CardContent to avoid backdrop-filter stacking context */}
            <button
              onClick={handleEmailClick}
              aria-label={`Email ${displayName}`}
              className="absolute bottom-2 left-2 z-10 p-1 rounded-full transition-transform hover:scale-110"
            >
              <EnvelopeSimpleIcon
                size={22}
            ...
            {/* Like button — bottom-right, sibling of CardContent to avoid backdrop-filter stacking context */}
            <button
